...
int main() {
	...
	time("calculating grid", [&]{
		parallel.foreach<RangeObj<spatialDim>::iterator, std::function<void(const Vector<int, spatialDim>&)>>(
			range.begin(),
			range.end(),
			[&](const Vector<int, spatialDim>& index) {
	...
	auto calc_GammaULLs = [&](const real* x) {
		parallel.foreach<RangeObj<spatialDim>::iterator, std::function<void(const Vector<int, spatialDim>&)>>(
			...
			[&](const Vector<int, spatialDim>& index) {
				...
				for (int a = 0; a < dim; ++a) {
					for (int b = 0; b < dim; ++b) {	
						dgLLL(a,b,0) = 0;	//TODO time derivative? 
						for (int i = 0; i < spatialDim; ++i) {
	...
	time("calculating gravitational force", [&]{
		...
		for (RangeObj<spatialDim>::iterator iter = range.begin(); iter != range.end(); ++iter) {
			...
			std::cout << iter.index 
				...
				<< " analyticalGravity=" << fabs(analyticalGravity)
				<< std::endl;
		}
	});
